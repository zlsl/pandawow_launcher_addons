...
local _string_find = string.find --> lua local
local _string_gsub = string.gsub --> lua local
...
function DF:NewPanel(parent, container, name, member, w, h, backdrop, backdropcolor, bordercolor)
	if not name then
		name = "DetailsFrameworkPanelNumber"..(self.PanelCounter)
	...
	if _string_find(name, "$parent") then
		name = _string_gsub(name, "$parent", parent:GetName())
	end
	...
	PanelObject.frame = CreateFrame("Button", name, parent)
	PanelObject.frame:SetSize (100, 100)
	PanelObject.frame.Gradient = {
		["OnEnter"] = {0.3, 0.3, 0.3, 0.5},
		["OnLeave"] = {0.9, 0.7, 0.7, 1}
	}
	PanelObject.frame:SetBackdrop ({
		bgFile = [[Interface\DialogFrame\UI-DialogBox-Background]],
		edgeFile = [[Interface\DialogFrame\UI-DialogBox-Border]], edgeSize = 10, tileSize = 64, tile = true
	})
	...
	PanelObject.frame:SetSize(w or 100, h or 100)
...
function DF:IconPick (callback, close_when_select, param1, param2)
	...
	if (not DF.IconPickFrame) then
		...
		DF.IconPickFrame.preview:SetFrameLevel (DF.IconPickFrame:GetFrameLevel() + 10)
		...
		local scroll = CreateFrame ("ScrollFrame", "DetailsFrameworkIconPickFrameScroll", DF.IconPickFrame, "FauxScrollFrameTemplate")
...
local feedback_get_fb_line = function (self)
	...
	if (not line) then
		line = CreateFrame ("Button", "AddonFeedbackPanelFB" .. self.next_feedback, self)
...
local feedback_get_addons_line = function (self)
	...
	if (not line) then
		...
		line = CreateFrame ("Button", "AddonFeedbackPanelSA" .. self.next_addons, self)
		line:SetSize (128, 64)
...
function DF:CreateKeybindBox (parent, name, data, callback, width, height, line_amount, line_height)
	...
	for index, specId in ipairs (specIds) do
		...
		selectedTexture:SetTexture (1, 1, 1, 0.5)
	...
	for i = 1, SCROLL_ROLL_AMOUNT do
		local f = CreateFrame ("Button", "$KeyBindFrame" .. i, keybindScroll)
		f:SetSize (1009, 20)
...
DF.IconRowFunctions = {
	...
	GetIcon = function (self)
		...
		if (not iconFrame) then
			...
			newIconFrame.Border:SetTexture (0, 0, 0)
...
function DF:BuildStatusbarAuthorInfo (f, addonBy, authorsNameString)
	...
	discordTextEntry:SetText ("https://discord.gg/m3YyWq7")
...
DF.TimeLineBlockFunctions = {
	...
	GetBlock = function (self, index)
		...
		if (not block) then
			block = CreateFrame ("Button", nil, self)
...
DF.TimeLineFunctions = {
	...
	GetLine = function (self, index)
		...
		if (not line) then
			--create a new line
			line = CreateFrame ("Button", "$parentLine" .. index, self.body)
