...
DBM_CORE_SLASHCMD_HELP				= {
	...
	"/dbm timer/ctimer/ltimer/cltimer <x> <text>: Запускает <x> сек. DBM таймер с именем <text>. Посмотреть как использовать каждый тип таймера можно на http://tinyurl.com/kwsfl59.",
	"/dbm broadcast timer/ctimer/ltimer/cltimer <x> <text>: Транслирует <x> сек. DBM таймер с именем <text> в рейд (требуются права лидера или помощника).",
